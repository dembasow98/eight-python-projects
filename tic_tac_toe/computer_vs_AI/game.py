...
print('\n----------------WELCOME TO TIC-TAC-TOE COMPUTER VS. AI GAME---------------------\n')

iteration = int(input('Enter the number of itarations you want the game to be played: '))
print('Wait, the guys are playing...')
#To get the time the execution time of the program
...
if __name__ == '__main__':
    ...
    for i in range(iteration):
        ...
        result = play( game,x_player, o_player,display=False)
        if result == 'X':
            x_wins += 1
        elif result == 'O':
            o_wins += 1
    ...
    print(f'After playing {iteration} times:')
    print(f'     -The computer has won {x_wins} time(s).')
    print(f'     -The AI has won {o_wins} time(s).')
    print(f'     -They tied {ties} time(s).')
